...
# input profit goal and product count
...
# if revenue <= 0, skip this product
    if revenue <= 0:
...
print("\n===== KPI REPORT =====")
for name, data in results.items(): # product name and {profit, margin}
...
# set the base values, so anything can not lower/higher
best_product = None
worst_product = None
best_margin = float('-inf')
worst_margin = float('inf')

# if current product margin is higher/lower than highest/lowest so far, update best/worst product
for name, data in results.items():
    if data['margin']>best_margin: best_margin=data['margin'];best_product=name
    if data['margin']<worst_margin: worst_margin=data['margin'];worst_product=name
...
print("Goal reached!" if total_profit > profit_goal else f"Missed profit goal by {profit_goal - total_profit:.2f}$")
# endregion